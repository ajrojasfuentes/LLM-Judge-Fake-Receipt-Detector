"""Voting engine with simple majority decision and rich uncertainty telemetry."""
...
class VotingEngine:
    """Simple-majority aggregator that preserves uncertainty diagnostics."""
    ...
    def _majority_vote(self, vote_counts: dict, n: int) -> tuple[str, str]:
        if vote_counts.get("UNCERTAIN", 0) >= self.uncertain_threshold:
            return "UNCERTAIN", "majority_simple (uncertain threshold)"
        ...
        if fake_votes >= majority:
            return "FAKE", "majority_simple"
        if real_votes >= majority:
            return "REAL", "majority_simple"
        return "UNCERTAIN", "majority_simple (no majority)"
    ...
    def _compute_verdict_uncertainty_simple(
        ...
        if n == 0:
            return 1.0
        disagreements = sum(1 for r in results if r.label not in (winner_label, "UNCERTAIN"))
