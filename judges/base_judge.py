"""Base judge contract and robust JSON parsing."""
...
class JudgeResult:
    ...
    def error_result(cls, judge_id: str, judge_name: str, receipt_id: str, error: str) -> "JudgeResult":
        return cls(
            ...
            label="UNCERTAIN",
            confidence=0.0,
            reasons=["Failed to parse judge response", "No reliable structured output returned"],
            skill_results={
                k: "uncertain"
                for k in [
                    "math_consistency",
...
class BaseJudge(ABC):
    ...
    def judge(
        ...
        receipt_id: str,
        image_path: Path,
        forensic_context: Optional[str] = None,
        extra_image_paths: Optional[Sequence[Path]] = None,
    ) -> JudgeResult:
        ...
        if forensic_context:
            prompt = str(forensic_context).strip() + "\n\n" + prompt

        all_images: list[Path] = [Path(image_path)]
        ...
        for _attempt in range(1, self.MAX_RETRIES + 1):
            try:
                raw = self._call_api(prompt=prompt, image_paths=dedup_images)
